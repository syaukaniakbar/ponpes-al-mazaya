<x-dashboard-layout>
    ...
    <div class="max-w-2xl p-6 mx-auto bg-white rounded-md shadow-md">
        ...
        <form action="{{ route('nav-links.update', $navLink->id) }}" method="POST" enctype="multipart/form-data" onsubmit="submitForm()">
            @csrf
            @method('PUT')
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <input
                    ...
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    placeholder="Masukkan Nama Link"
                    value="{{ old('content', $navLink->content ?? '') }}">
                            ...
                            <div class="w-full">
                                ...
                                <script>
                                    document.addEventListener("DOMContentLoaded", function() {
                                        const iframe = document.querySelector('[x-ref="wysiwyg"]');
                                        const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;

                                        iframeDocument.open();

                                        // Get the content from Blade and clean it up
                                        let content = `{!! old('content', $navLink->content ?? '') !!}`;

                                        // Remove unwanted newlines, carriage returns, and trim the content
                                        content = content.replace(/[\n\r]+/g, ' ').trim(); // Replaces multiple newlines with spaces and trims
                ...
                <div class="mb-6">
                    ...
                    <input
                        ...
                        value="1"
                        class="rounded-md"
                        {{ old('is_active', $navLink->is_active ?? '') ? 'checked' : '' }}>
                    @error('is_active')
